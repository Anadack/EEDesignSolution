...
> Define systems once in JSON, auto-map to ECU hardware, validate 22 architecture rules plus zone-integrity and DBC-level checks, estimate sizing, generate a full documentation suite — all from a single build command.
...
6. [Pre-Validation Engine — 22 Checks](#6-pre-validation-engine--22-checks)
...
System JSON library  →  C compilation  →  Verification  →  Export  →  HTML documentation
      (library/)           (src/ + inc/)      (22 rules)    (generated_doc/exports/)    (tools/scripts/)
```
...
- Executes **22 architectural verification rules** (V1–V13, B1–B9), plus zone integrity (Z1–Z3) and DBC-level checks (D1–D8) and produces a PASS/WARN/FAIL report
...
│  src/EEC_verify.c    — 22 verification rules (V1–V13, B1–B9)       │
...
│  verify_report.txt                   (22-rule verification result)  │
└────────────────────────────┬────────────────────────────────────────┘
                             │ consumed by
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
...
Builds and runs every hermetic test in `qa/` against the real engine (no mocks): library import/export round-trips, DBC import/export round-trip, the ES3 generic-ECU pinout import, and the new B9 busload / DBC-validator unit tests. Each test links against the full `src/EEC_*.c` set and uses only `tmpfile()` / a portable temp directory, so it runs unmodified on any machine and from CI. Naming-convention-specific tests can also be run standalone with `./qa/run_naming_tests.sh`.
...
## 6. Pre-Validation Engine — 22 Checks

Every build executes 22 architectural rules before exporting (13 signal-level + 9 bus-level), plus two independent rule families that run at export time: zone integrity (Z1–Z3, ZONAL mode only) and DBC-level validation (D1–D8, CAN/ISOBUS buses only). This prevents design errors from reaching CAD tools, wiring harness design, or hardware procurement.
...
| **Verification Report** | 22-rule pass/fail/warn (text file + embedded in overview) |
...
| **Pre-validation before hardware** | 22 rules catch electrical mismatches, missing diagnostics, ground mixing, bus errors at compile time |
...
| **Bug tracking** | 22 rules as a non-regression CI gate; failures identify the exact rule, signal, ECU, and pin |
...
| **Validated** | Compliance check | 22 verification rules, safety trace, diagnostics matrix |
...
│   ├── EEC_verify.c             22 verification rules (V1–V13, B1–B9)
...
| Feature | Scope | Impact |
|---------|-------|--------|
| **Bandwidth Analysis (LIN / Ethernet)** | Calculate bus load per LIN/ETH message, warn on overload — CAN/ISOBUS busload is shipped as rule B9 (ISO 11898-1 worst-case estimate, see [Section 6](#6-pre-validation-engine--22-checks)) | Prevent runtime communication failures |
