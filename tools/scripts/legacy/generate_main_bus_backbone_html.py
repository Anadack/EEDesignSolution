...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the AM demo, run it, and generate the bus backbone HTML view.")
    ...
    base = find_base()
    generator_script = base / "tools" / "generate_architecture_bus_backbone_html.py"
    app_path = base / args.app_name
    json_path = base / "exports" / "example_physical_architecture.json"
    html_path = base / "exports" / "architecture_bus_backbone.html"
    ...
    run_command([args.python, str(generator_script.relative_to(base))], base, "Generate bus backbone HTML from exported JSON")
