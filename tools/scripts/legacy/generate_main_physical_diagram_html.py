...
def build_demo(base: Path, compiler: str, app_name: str) -> Path:
    ...
    run_command(command, base, "Build demo executable from src/main.c")
    return app_path


def run_demo(base: Path, app_path: Path) -> None:
    run_command([str(app_path)], base, "Run demo executable to export physical architecture JSON")


def run_html_generator(base: Path, python_executable: str, generator_script: Path) -> None:
    run_command([python_executable, str(generator_script.relative_to(base))], base, "Generate physical pinout HTML from exported JSON")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the AM demo application, run it, and generate the physical HTML pinout view."
    )
    parser.add_argument(
        "--compiler",
        default="C:/mingw64/bin/gcc.exe",
        help="C compiler used to build the demo executable.",
    )
    parser.add_argument(
        "--python",
        default=sys.executable,
        help="Python interpreter used to run the HTML generator.",
    )
    parser.add_argument(
        "--app-name",
        default="app.exe" if sys.platform.startswith("win") else "app",
        help="Output demo executable name.",
    )
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Reuse the existing demo executable instead of recompiling it.",
    )
    parser.add_argument(
        "--skip-run",
        action="store_true",
        help="Reuse the existing example_physical_architecture.json instead of rerunning the demo.",
    )
    args = parser.parse_args()

    base = find_base()
    generator_script = base / "tools" / "generate_architecture_html_pinout.py"
    json_path = base / "exports" / "example_physical_architecture.json"
    html_path = base / "exports" / "architecture_pinouts_stacked.html"
