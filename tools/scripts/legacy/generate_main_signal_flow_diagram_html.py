...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the AM demo, run it, and generate the signal flow HTML view.")
    ...
    base = find_base()
    generator_script = base / "tools" / "generate_architecture_signal_flow_html.py"
    app_path = base / args.app_name
    json_path = base / "exports" / "example_architecture.json"
    html_path = base / "exports" / "architecture_signal_flow.html"
    ...
    run_command([args.python, str(generator_script.relative_to(base))], base, "Generate signal flow HTML from exported JSON")
